...
class BoardTestSuite {
    ...
    void testAddTaskListAverageWorkingOnTask(){
        ...
        long numberOfTasksInProgress = project.getTaskLists().stream()
                ...
                .map(Task::getCreated)
                .count();
        long numberOfDaysSinceTaskStarted = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
        ...
        long average1stMethod = numberOfDaysSinceTaskStarted / numberOfTasksInProgress;

        double average2ndMethod = project.getTaskLists().stream()
                .filter(inProgressTasks::contains)
                .flatMap(tl -> tl.getTasks().stream())
                .map(Task::getCreated)
                .map(d -> d.until(LocalDate.now(), ChronoUnit.DAYS))
        ...
        //Then
        assertEquals(10.0, average1stMethod, 0.0001);
        assertEquals(10.0, average2ndMethod, 0.0001);
    }
}